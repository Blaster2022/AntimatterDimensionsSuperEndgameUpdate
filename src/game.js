...
export function gameLoop(passedDiff, options = {}) {
  ...
  updateEterigaryMachines(realDiff);
  ...
  player.celestials.laitela.darkMatter = SingularityMilestone.uncapDark.isUnlocked ? new Decimal(darkMatterProd) : Decimal.min(darkMatterProd, Laitela.darkMatterCap);
...
function updateEterigaryMachines(diff) {
  MachineHandler.updateEMCap();
  Currency.eterigaryMachines.add(MachineHandler.gainedEterigaryMachines(diff));
}
...
export function gainedDoomedParticles() {
  if (!player.break2) return DC.D1;
  let dp = player.records.totalEndgameAntimatter.add(1).log10().div(9e15);
  return dp.floor();
}
