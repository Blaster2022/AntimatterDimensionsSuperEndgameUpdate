...
export const singularityMilestones = {
  ...
  singularityPower: {
    start: Decimal.pow(10, 50000),
    repeat: new Decimal(0),
    limit: 1,
    description: "Celestial Points provide a power effect to Singularities",
    effect: () => Currency.celestialPoints.value.max(10).log10().log10().pow(2).div(10).add(1).toNumber(),
    effectFormat: x => formatPow(x, 2, 3),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  celPointBoost: {
    start: Decimal.pow(10, 150000),
    repeat: new Decimal(0),
    limit: 1,
    description: "Boost Celestial Point gain based on Singularities",
    effect: () => Currency.singularities.value.max(10).log10().log10().div(1e5).pow(0.5).toNumber(),
    effectFormat: x => formatPow(x, 2, 3),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_MAIN,
  },
  singBoostFromCosmicSector: {
    start: Decimal.pow(10, 300000),
    repeat: new Decimal(0),
    limit: 1,
    description: "Singularities gain a power effect from Cosmic Sector",
    effect: () => Math.pow(player.endgame.ethereal.sector, 0.2),
    effectFormat: x => formatPow(x, 2, 3),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  iMBoost: {
    start: Decimal.pow(10, 600000),
    repeat: new Decimal(0),
    limit: 1,
    description: "Empower Imaginary Machine gain based on Singularities",
    effect: () => Currency.singularities.value.max(10).log10().log10().div(1e5).pow(0.2).toNumber(),
    effectFormat: x => formatPow(x, 2, 3),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_MAIN,
  },
  uncapDark: {
    start: Decimal.pow(10, 1e6),
    repeat: new Decimal(0),
    limit: 1,
    description: "Uncap Dark Matter",
    effect: completions => completions.toNumber(),
    effectFormat: x => (x === 1 ? "Unlocked" : "Locked"),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.SELF_BOOST,
  }
